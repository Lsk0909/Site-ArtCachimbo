...
echo.
echo    1️⃣  LIMPAR TODOS OS LOGINS E DADOS
echo    2️⃣  ABRIR SITE E CONSOLE (RECOMENDADO)
echo    3️⃣  CRIAR USUÁRIO KLEBER COMO ADMIN
echo    4️⃣  VER COMANDOS DO CONSOLE
echo    5️⃣  ABRIR PAINEL ADMINISTRATIVO
echo    6️⃣  EXECUTAR SCRIPT PYTHON
echo    7️⃣  SAIR
echo.
...
if "%opcao%"=="2" goto abrir_site_console
if "%opcao%"=="3" goto criar_kleber
if "%opcao%"=="4" goto ver_comandos
if "%opcao%"=="5" goto painel_admin
if "%opcao%"=="6" goto script_python
...
:limpar_tudo
echo.
echo 🧹 LIMPANDO TODOS OS DADOS DO SITE...
echo.
echo ⚠️  ATENÇÃO: Esta ação irá:
echo    • Apagar TODOS os usuários
echo    • Limpar TODOS os logins
echo    • Remover TODAS as visitas
echo    • Resetar o sistema completo
echo.
echo    🔄 Após limpar, você poderá criar novos usuários
echo.
set /p confirm="Tem certeza que deseja continuar? (S/N): "
if /i "%confirm%"=="S" (
    echo.
    echo 🌐 Abrindo página para limpar dados...
    python limpar_todos_logins.py
    if errorlevel 1 (
        echo ❌ Erro ao executar script Python
        echo 💡 Alternativa: Use o console do navegador (F12)
        echo    Digite: localStorage.clear()
        echo.
        echo 🌐 Abrindo site...
        start http://localhost:8000/index.html
        echo.
        echo ✅ No site, pressione F12 e digite: localStorage.clear()
    )
) else (
...
echo.
if exist limpar_todos_logins.py (
    echo ✅ Script de limpeza encontrado! Executando...
    python limpar_todos_logins.py
) else (
    echo ❌ Script limpar_todos_logins.py não encontrado!
